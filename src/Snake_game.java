...
public class Snake_game extends JPanel implements ActionListener, KeyListener{
    ...
    public void draw(Graphics g){
        ...
        // score
    Font scoreFont = new Font("Verdana", Font.BOLD, 20);
    g.setFont(scoreFont);
    if (gameOver) {
        g.setColor(Color.red);
        g.drawString("Game Over: " + String.valueOf(snakeBody.size()), tileSize, tileSize);
    } else {
        g.setColor(Color.blue); // Choose a color that contrasts well with your background
    ...
    public void move(){
        ...
        if (snakeHead.x *tileSize< 0 || snakeHead.x*tileSize >boardWidth || snakeHead.y < 0 || snakeHead.y*tileSize <0|| snakeHead.y*tileSize>boardHeight) {
            gameOver= true;
        }
